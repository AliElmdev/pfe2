    ...
    <section class="clean-block clean-form dark">
        ...
        <ul class="nav nav-tabs">
            ...
            <li class="nav-item">
                ...
                <a class="nav-link" href={{route('chats',[$id])}} style="background: #3c8447; color: #ffffff; box-shadow: 0px 0px; border-radius: 10px;
        ...
        <form style="max-width:100%" action="{{route('postulations',$marche[0]->id)}}" method="POST"
            ...
            <div class="container" style="padding-right: 0px; padding-left: 0px;">
                <div style="
                    ...
                    <p style="font-family: Roboto, sans-serif;">
                        Dossier :<a
                            href="https://esuppliers.ocpgroup.org/esop/toolkit/negotiation/tnd/detailTender.do?selectedCommandName=SETTINGS&amp;tenderCode=tender_45283&amp;from=menu"><strong>Dossier_48865</strong></a>-
            ...
            <div class="text-warning d-flex d-lg-flex flex-row justify-content-end align-self-end"
                ...
                <button class="btn btn-primary modifer" type="button"
                    style="margin-right: 9px; background: var(--bs-success); display:block;"
                    onclick="btn_modifer();">Modifier</button><button class="btn btn-primary enregister" type="submit"
                    style="margin-right: 9px; background: var(--bs-success);display:none"
                    onclick="btn_modifer();">Enregister</button><button class="btn btn-primary annuler" type="submit"
                    style="background: var(--bs-green);" onclick="annuler();">Annuler</button>
            </div>
            ...
            <div style="margin-top: 26px;">
                <h2 style="font-family: Roboto, sans-serif; margin-left: 17px; font-size: 21.128px;">
                    <strong>{{$b_section->nom}}</strong><br />
                </h2>
                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header panel-title mb-0" role="tab">
                            <button type="button" class="accordion-button collapsed collapsed" data-bs-toggle="collapse"
                                data-bs-target="#accordion-{{$b_section->id}} .item-{{$section->id}}"
                                aria-expanded="true"
                                aria-controls="accordion-{{$b_section->id}} .item-{{$section->id}}">
                                <i class="fa fa-comments"></i>{{$section->nom_section}}<br />
                            </button>
                        </h2>
                        <div class="accordion-collapse collapse item-{{$section->id}}" role="tabpanel"
                            data-bs-parent="#accordion-{{$b_section->id}}">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="upload-btn-wrapper">
                                                        <button name="hnea"
                                                            class="btn_upload reponseqst_{{$question->id}}">Télécharger
                                                            un fichier</button>
                                                        <input name="reponseqst_{{$question->id}}" type="file"
                                                            class="form-costum reponse_l reponse_file reponseqst_{{$question->id}}"
                                                            disabled />
                                                    </div>
                                                    {{-- <input name="reponseqst_{{$question->id}}" type="file"
                                                        class="form-costum reponse_l reponseqst_{{$question->id}}"
                                                        disabled /> --}}
                                                </td>
                                                @elseif($question->type == 'cm')
                                                <td>{{$question->question}}</td>
                                                <td>
                                                    <select name="reponseqst_{{$question->id}}"
                                                        class="reponse_l reponseqst_{{$question->id}}"
                                                        style="height: 25.8px; width: 134px;" disabled>
                                                        <optgroup label="Oui / NON">
                                                ...
                                                <td>{{$question->question}}</td>
                                                <td><input name="reponseqst_{{$question->id}}" type="text"
                                                        class="form-costum reponse_l reponseqst_{{$question->id}}"
                                                        disabled /></td>
                ...
                <div class="accordion" role="tablist" id="accordion-3">
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td style="border-width: 0px;"><input class="reponse_l" disabled
                                                        type="file" /></td>
                    ...
                    <div class="accordion-item">
                        ...
                        <div class="accordion-collapse collapse show item-2" role="tabpanel"
                            ...
                            <div class="accordion-body">
                                <div class="table-responsive"
                                    style="margin-left: 2px; margin-right: 25px; border: 2.4px none var(--bs-blue);">
                                    <table class="table">
                                        ...
                                        <tbody style="padding-left: 10px;">
                                            ...
                                            <tr style="text-align:center;">
                                                ...
                                                <td>
                                                    <select name="type_{{$produit->id}}"
                                                        class="reponse_l type_{{$produit->id}}" disabled>
                                                        <optgroup label="Type">
                                                            <option value="0" selected=""></option>
                                                ...
                                                <td>
                                                    <select name="devis_{{$produit->id}}"
                                                        style="height: 22.8px; width: 105.4px;"
                                                        class="reponse_l devis_{{$produit->id}}" disabled>
                                                        <optgroup label="Type">
                                                            <option value="0" selected=""></option>
                                                ...
                                                <td><input name="prix_{{$produit->id}}"
                                                        class="reponse_l prix_{{$produit->id}}" disabled
                                                        onkeyup="calcule_prix_totale({{$produit->qte}},this,{{$produit->id}})"
                                                        type="number" id={{$produit->id.'prix_input'}} style="width:
                                                    77.4px; border-radius: 0px; border-width: 1px; border-style: solid;"
                                                    /></td>
